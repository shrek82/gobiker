...
class AttachedsController < ApplicationController
  ...
  def upload
    ...
    if @attached.save
      ...
      respond :state=>'SUCCESS',:url=>file_path,:file_id => @attached[:id], :fileName => @attached[:img_file_name],:fileType=>@attached[:img_content_type],:fileSize => @attached[:img_file_size], :success => '资料修改成功'
    else
