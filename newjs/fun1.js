...
//----------------------------------------------------------
//测试1、对象直接作为普通函数执行
MyClass('http://test1.com', '测试1、对象直接作为普通函数执行');
window.username; //ok
window.send(); //ok
...
//测试2、对象当作普通函数被其他对象引用
var class1 = MyClass('http://test2.com', '测试2、对象当作普通函数被其他对象引用');
window.username; //ok
window.send(); //ok
console.log(class1.username);//当无论Myclass是否返回本身，函数体内this都指向window
class1.send();