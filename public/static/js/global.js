...
var process_submit = (function () {
    var proc = function () {
        ...
        this.addclass = null;
        this.isval = true;
        this.before = function (id, rmclass, addclass, val, isval) {
            this.id = id;
            ...
            if (this.isval) {
                jQuery("#" + id).removeClass(rmclass).addClass(addclass).val(val);
            } else {
                jQuery("#" + id).removeClass(rmclass).addClass(addclass).html(val);
            }
        };
        this.after = function (val) {
            if (this.isval) {
                jQuery("#" + this.id).removeClass(this.addclass).addClass(this.rmclass).val(val);
            } else {
                jQuery("#" + this.id).removeClass(this.addclass).addClass(this.rmclass).html(val);
            }
        }
    };
    return new proc();
...
(function () {
    var fullpup = {};
    var isfirst = true;
    var codehtml = "<div class='ui_fullpupBox_bg'><div class='ui_fullpupBox'><div class='ui_fullpupBox_main'></div></div></div>"

    function fullpupClose() {
        jQuery("html").css({
            "overflow": "",
            "height": ""
        });
        ...
        jQuery(".ui_fullpupBox_bg").hide();
        isfirst = false;
        return false;
    }

    function fullpupStart(width) {
        if (isfirst) {
            jQuery("body").append(codehtml);
        }
        isfirst = false;
        jQuery("html").css({
            "overflow": "hidden",
        ...
        jQuery(".ui_fullpupBox_bg").css({
            "display": "block",
            "height": jQuery(window).height()
        });
        jQuery(".ui_fullpupBox").css({
            "width": width
        });
        jQuery(".ui_fullpupBox_main").text("");
    }

    jQuery(".ui_fullpupBox_close").live("click", function () {
        fullpupClose();
    });
    fullpup.ajax = function (obj) {
        var url = obj.url;
        var width = obj.width;
        fullpupStart(width);
        jQuery.get(url, function (html) {
            jQuery(".ui_fullpupBox_main").html(html);
            jQuery(".ui_fullpupBox_head").css({
                "width": width
            });
        });
    };
    fullpup.show = function (obj) {
        var id = obj.id;
        var width = obj.width;
        fullpupStart(width);
        var idText = jQuery("#" + id).html();
        jQuery(".ui_fullpupBox_main").html(idText);
        jQuery(".ui_fullpupBox_head").css({
            "width": width
        });
    }
    qyerUI.fullpup = fullpup;
})();
(function ($) {
    var popup = {};
    var initial = true;
