module Gobiker
  module Generators
    ...
    class ViewsGenerator < Rails::Generators::NamedBase
      source_root File.expand_path('../../../../app/views/gobiker', __FILE__)
      ...
      def self.banner #:nodoc:
        <<-BANNER.chomp
rails g gobiker:views THEME [options]
...
#{themes.map { |t| "        - #{t.name}\n#{t.description}" }.join("\n")}
      ...
      def download_templates(theme)
        theme.templates_for(template_engine).each do |template|
          say "      downloading #{template.name} from gobiker_themes..."
      ...
      def copy_default_views
        ...
        Dir.glob(filename_pattern).map { |f| File.basename f }.each do |f|
          copy_file f, "app/views/gobiker/#{f}"
        end
      end
    ...
    module GitHubApiHelper
      def get_files_in_master
        master_tree_sha = open('https://api.github.com/repos/amatsuda/gobiker_themes/git/refs/heads/master') do |json|
          ActiveSupport::JSON.decode(json)['object']['sha']
        end
        open('https://api.github.com/repos/amatsuda/gobiker_themes/git/trees/' + master_tree_sha + '?recursive=1') do |json|
      ...
      def get_content_for(path)
        open('https://api.github.com/repos/amatsuda/gobiker_themes/contents/' + path) do |json|
